...
@Slf4j
@AllArgsConstructor
public class CipherFeedback implements Mode {
    ...
    public byte[] encryption(byte[] block) throws IllegalArgumentException, ExecutionException, InterruptedException {
        ...
        log.info("Start encryption mode CFB");
        ...
        for (int i = 0; i < block.length; i += cipher.getSizeBlock()) {
            ...
            byte[] pmt = xor(cipher.encryption(InitVector), temporaryBlock);
    ...
    public byte[] decoding(byte[] block) throws IllegalArgumentException, ExecutionException, InterruptedException {
        ...
        log.info("Start decoding mode CFB");

        return new TextTaskRunConcrete(new ThreadModeCfbDecrypt(vector, cipher), cipher.getSizeBlock()).threadTextRun(block);
    }
}
