...
public class ThreadModeRdEncrypt implements ThreadMode {
    private final CipherInterface cipher;
    private final byte[][] counter;

    public ThreadModeRdEncrypt(byte[] vector, CipherInterface cipher, byte[] block) {
        this.cipher = cipher;
    ...
    public Pair<Integer, byte[]> run(int countBlock, int blockSize, byte[] array, int index) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < countBlock; i++) {
            ...
            byte[] encryptBlock = cipher.encryption(xor(block, counter[(index + i * blockSize) / blockSize]));
